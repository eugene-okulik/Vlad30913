...
class EcoFriendly(BasePage):
    ...
    def sort_by_price_and_check_correct_sort(self, text):
        first_man = self.page.locator(ecoloc.products_loc).locator('nth=0')
        print(first_man.inner_text())
        self.page.locator(ecoloc.sorted_loc).locator('nth=0').select_option('Price')
        expect(self.page).to_have_url(re.compile(text))
        print(first_man.inner_text())

    def enter_search_text(self, text):
        search_box = self.page.locator(ecoloc.search_box_loc)
    ...
    def check_filter_products_by_price(self, text):
        expect(self.page.locator(ecoloc.filtered_loc)).to_have_text(text)
